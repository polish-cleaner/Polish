...
- **Full Dev/AI catalog** — Pro-only: WSL2 VHDX (wrapper over `wsl --manage --resize` — already GA in WSL 2.5+), Docker VHDX (wrapper over `Optimize-VHD`), LM Studio/Ollama model audit, IDE caches, go-build, orphan node_modules detection
...
> **`ARCHITECTURE.md` (root) is authoritative for code structure, layering, and patterns** (hexagonal/ports-and-adapters, typestate pipeline, `inventory` distributed category registration, `thiserror`/`anyhow` discipline, CI gates). This section covers process model + IPC + signing — orthogonal to code architecture. If §4 and ARCHITECTURE.md conflict on layout/patterns, ARCHITECTURE.md wins.
...
> **Superseded by `ARCHITECTURE.md` §2.** The layout below was the pre-architecture-research thinking (`apps/ui` + `apps/svc` + `crates/polish-ipc` + `polish-scanner` + `polish-quarantine` + `polish-rules` + `polish-platform`). The canonical workspace structure is now hexagonal: `crates/polish-core` + `polish-app` + `polish-adapters` + `polish-ipc` + `polish-test-fixtures` + bins (`polish-svc`, `polish-ui`, `polish-cli`, `polish-pro`). See ARCHITECTURE.md §2 for full layout and dependency direction rules. Block kept here for historical context — do not build to this layout.
...
> **Updated post-research 2026-05-28.** Primary path switched from SignPath Foundation to **Microsoft Artifact Signing** (formerly Trusted Signing) per canonical [MS Learn SmartScreen guidance, updated 2026-05-09](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/smartscreen-reputation): identity-validated individual accounts are now supported, ~$10/mo, no hardware token, native GitHub Actions / Azure DevOps integration. Previous "3-yr org rule" assumption is outdated. Reputation accumulates the same as any other valid cert — switching issuer does not skip seasoning.

**Decision:** use **Microsoft Artifact Signing** (primary). ~$10/mo, no token, identity-validated, CI/CD-native. Subject is the validated identity (individual or org). See [Artifact Signing docs](https://learn.microsoft.com/en-us/azure/trusted-signing/).

**Fallback 1 (if Artifact Signing identity validation blocks):** **Certum Open Source** cert (~€69 initial + €29/yr). Issued to individual; YOUR name is publisher. +1–2 weeks KMS-only CI plumbing.

**Fallback 2 (if both above fail):** **SignPath Foundation** (free for OSS, HSM-backed, GitHub Actions CI integration, 4–8 week processing). MIT license + active maintenance + reproducible builds qualifies. Used by Notepad++, Greenshot, AutoHotkey.

**Hard skips:**
- EV certificates ($279+/yr) — per MS canonical: "EV certificates no longer bypass SmartScreen... Paying a premium for EV solely to avoid SmartScreen warnings is no longer justified." Not worth the cost.
...
**Reality check for users:** expect SmartScreen warnings for weeks/months while reputation builds — per MS canonical, "several weeks and hundreds of clean installs from a wide audience." README will include a "Right-click → Properties → Unblock" guide for early adopters.

**Smart App Control (Win11) caveat (NEW post-research 2026-05-28):** SAC blocks unsigned/low-reputation binaries at execution, not just download, with no "Run anyway" override on SAC-enabled machines. Default-on for clean Win11 22H2+ installs. Both `polish-svc.exe` and `polish-ui.exe` MUST be signed from day 1; document SAC-disable workaround in onboarding until reputation builds.

**2026 deadline:** effective March 1 2026, max code-signing cert validity dropped from 39 months to ~460 days (CA/B Forum Ballot CSC-31). Re-issuance is now ~annual. Plan for it.
...
- **`release.yml`** on tag push (`v*`): full test + build matrix → sign via MS Artifact Signing GitHub Action (Certum/SignPath fallback) → NSIS bundle → upload to GitHub Releases → submit winget PR → publish Scoop manifest update.
...
- **Code signing + SmartScreen reputation is the real market entry tax**, not feature breadth. CCleaner / Wise / Glary all have multi-year signing reputation. New entrants face 3–6 months of SmartScreen seasoning during which install-completion rates run materially lower (no quantitative MS-published figure; multi-source qualitative). Smart App Control on Win11 makes the gate harder (no "Run anyway" override). EV certs no longer skip the gate (MS removed instant-trust ~2024). Built into v1.0 launch calendar (§20, §21).
...
- **TAM correction.** Real Windows PC utilities / optimization market is ~$1.05B / 5.8% CAGR (Mordor Intelligence, Apr 2024) — not "$1–3B / 8–10%." Trend is flat-to-declining, not growing, as Microsoft ships native cleanup primitives (Dev Drive in 23H2, `wsl --manage --resize` GA in WSL 2.5+, Storage Sense expanding).
...
- [ ] Signed installer (MS Artifact Signing primary; Certum / SignPath fallback) + signed binaries
...
|---|---|---|---|
| **v0.x** | Bootstrap + critical spikes | 2026 Q3 | Repo bootstrap, IPC scaffolding, **Tauri service-update spike (Week 1–3 — kill switch)**, signing onboarding (MS Artifact Signing primary; Certum / SignPath fallback), trademark search, founder runway gate |
...
- MS Artifact Signing onboarded (or Certum / SignPath fallback in place) by month 3
- Microsoft not shipping auto-shrink-on-idle WSL + Storage Sense dev-aware before v1.2 (else Dev/AI wedge pivots — see §22). `wsl --manage --resize` already GA in WSL 2.5+; wedge window tightened to 6–18 months per research 2026-05-28.
...
| **SmartScreen reputation seasoning (3–6 month install-friction window)** (NEW post-debate; EV-cert framing dropped per research 2026-05-28) | **High / Medium** | Sign dev builds from month 1 with MS Artifact Signing (primary) or Certum (fallback). Submit to MS Defender AV trust early. README "Unblock" guide. **No Pro launch until SmartScreen seasoned (~month 6+).** Switching issuer (Certum / SignPath / Artifact Signing) does NOT inherit reputation — new-cert friction lasts 3–6 months regardless. |
| **Smart App Control (Win11) blocks low-rep binaries at execution** (NEW post-research 2026-05-28) | **High / Medium** | SAC is default-on for clean Win11 22H2+ installs. Applies to all `.exe` at runtime (not just downloads); no user "Run anyway" override. Both `polish-svc.exe` and `polish-ui.exe` MUST be signed from day 1. Document SAC-disable workaround in onboarding for early adopters until reputation builds. Seasoning gate is the same as SmartScreen but with no escape hatch. |
| **Microsoft ships native parity for WSL2 dev wedge before v1.2** (`wsl --manage --resize` already GA in WSL 2.5+; auto-shrink-on-idle plausible 12mo roadmap — GitHub WSL #4699 open since 2019) (UPGRADED 12-24mo → 6-18mo per research 2026-05-28) | **Medium-High / High** | Architect WSL2 wrapper over `wsl --manage --resize` (current canonical command), not `wsl --shrink`. Kill-switch scenario: if MS ships auto-shrink-on-idle before v1.2, pivot Polish to Format Prep + general cleanup, drop dev-tool framing. |
| **Microsoft adds Storage Sense dev-cache awareness** (Docker VHDX, npm/pnpm/cargo, LM Studio/Ollama auto-cleanup) | **Medium / High** | 12–24 month horizon (no current MS docs signal imminent). Wrapper-first architecture for all dev categories; design Format Prep + atomic quarantine to stand alone if dev wedge collapses. |
...
| MS Artifact Signing identity validation may block (or service may be unavailable) | Low-Medium / Medium | Certum Open Source fallback (~€69 upfront + €29/yr + 1–2 weeks KMS-only CI plumbing). SignPath Foundation as tertiary fallback (4–8 week processing, free for OSS). Reputation does NOT inherit across issuers regardless of choice. |
...
| WSL distro unregister is destructive | High / Fatal | NEVER auto-checked. Require export-first flow. Disable in Light/Balanced. Pro-tier only (v1.2+) and wraps `wsl --manage --resize` (non-destructive, GA in WSL 2.5+) — not the older `wsl --shrink` framing. |
...
### Code signing
- [Microsoft Artifact Signing (formerly Trusted Signing) docs](https://learn.microsoft.com/en-us/azure/trusted-signing/) — **primary**, ~$10/mo, identity-validated, CI/CD-native, no token (3-yr org rule no longer applies per 2026-05 MS Learn updates)
- [Azure Artifact Signing pricing & FAQ](https://azure.microsoft.com/en-us/pricing/details/artifact-signing/)
- [SmartScreen reputation (Microsoft Learn, updated 2026-05-09)](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/smartscreen-reputation) — canonical: EV no longer bypasses; Smart App Control caveat for Win11
- [Certum Open Source Code Signing](https://certum.store/open-source-code-signing-code.html) — fallback 1 (identity validation failure)
- [SignPath Foundation for OSS](https://signpath.io/solutions/open-source-community) — fallback 2 (free, HSM-backed, 4–8 week processing)
- [CA/B Forum Ballot CSC-31 — 460-day validity cap (March 2026)](https://cabforum.org/)
